...
//              Restricted strictly to the 'authority' role.
// =========================================================================
  ...
  try {
    ...
    if (supabaseUrl && supabaseAnonKey && !supabaseUrl.includes("your-project")) {
      ...
      userRole =
        ...
        "authority"; // Default to authority in demo mode so Aditya's UI button works immediately
    }
    ...
    for (const item of ZONE_SIMULATIONS) {
      ...
      // Synchronize in-memory store so Aditya's frontend map & queue update instantly
    ...
    return NextResponse.json(
      { error: "Internal server error triggering disaster simulation." },
      { status: 500 }
    );
  }
}
